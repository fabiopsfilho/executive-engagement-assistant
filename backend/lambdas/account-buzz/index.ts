...
4. NOW SECTION — CRITICAL: The NOW section must be DIRECTLY GROUNDED in the Buzz findings. Specifically:
   ...
   - "now_opening_move": Reference a specific signal (a role they're hiring for, something an employee said, an executive post, or industry context) and explain why leading with this angle works for THIS company. You may name real executives found in the data. Only call someone an attendee if they are in the Attendees list. Never invent a name, never claim someone is a confirmed attendee, and never add a disclaimer about a missing attendee list — just give the recommendation naturally.
